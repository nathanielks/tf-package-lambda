...
mkdir -p "${BUILD_DIR}" "$(dirname $OUTPUT_PATH)" >&3

rsync -a --delete --exclude 'node_modules' "${SOURCE_DIR}/" "${BUILD_DIR}/" >&3

NPM_PROGRESS="$(npm get progress)"

cd "${BUILD_DIR}"

npm ci --loglevel=error >&3

chmod -R 0755 . >&3
[[ -d node_modules/.bin ]] && chmod -R 0777 node_modules/.bin >&3
...
npx removeNPMAbsolutePaths "${BUILD_DIR}" >&3

deterministic-zip $OUTPUT_PATH "${BUILD_DIR}" >&3
